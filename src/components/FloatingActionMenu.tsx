...
type ItemVariant = 'nav' | 'action' | 'danger';
...
export function FloatingActionMenu(props: FloatingActionMenuProps) {
  ...
  const handleDeptSelect = (dept: UserDepartment) => {
    if (pendingAction) openActionModal(pendingAction, dept);
  };
      ...
      {open && (
        ...
        >
          ...
          <div className="p-2 grid grid-cols-3 gap-1.5">
            ...
            {isAdmin && (
              <MenuItem icon={Shield} label={t('sidebar.adminPanel')} active={isActive('/admin')} onClick={() => go('/admin')} />
            )}
          </div>
          ...
          {isLeader && (
            <>
              ...
              <div className="p-2 grid grid-cols-3 gap-1.5">
                ...
                <MenuItem icon={UserCog} label={t('sidebar.roles')} variant="action" onClick={() => handleContextualAction('roles')} />
                <MenuItem icon={Users} label={t('sidebar.teamSummary')} variant="action" onClick={() => handleContextualAction('schedule-count')} />
                <MenuItem icon={UserPlus} label={t('sidebar.inviteMember')} variant="action" onClick={() => handleContextualAction('invite')} />
                <MenuItem icon={FileDown} label={t('sidebar.export')} variant="action" onClick={() => handleContextualAction('export')} />
                <MenuItem icon={Wrench} label={t('sidebar.deptSettings')} variant="action" onClick={() => handleContextualAction('dept-settings')} />
              </div>
            </>
          )}

          <div className="px-2 py-2 border-t border-border/60 flex items-center justify-center gap-2">
            <ThemeToggle />
            <LanguageSelector />
            <NotificationBell />
          </div>
